...
if($cleanfiles =~ "TRUE"){
	...
	my $iterateJobName	= "Iterate_$myJobName2\_$expFolder";
...
#	unless( -d "$path2aligned" )    { `mkdir $path2aligned`;        }
	...
	# Add the next job line to the $QSUB
        foreach( @myJobs ){ $_ = "\$".$_ ; }
        my $myJobsVec   = join(":", @myJobs);
        my $finalcmd    = "RENAME=\`qsub -N $iterateJobNameRename -o $path2qsub -e $path2qsub -W depend=afterany\:$myJobsVec $QSUBintRename`";
